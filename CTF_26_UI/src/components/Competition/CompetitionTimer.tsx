...
export default function CompetitionTimer({
  ...
  return (
    <motion.div
      ...
      className={`p-4 sm:p-6 md:p-8 rounded-xl bg-linear-to-br ${getStatusColor(timeData.status)} border backdrop-blur-sm`}
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-6">
        <div className="flex items-center gap-2 sm:gap-3">
          <Clock className="w-5 sm:w-6 h-5 sm:h-6 text-emerald-400" />
          <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-emerald-300">
        ...
        <span
          className={`px-3 py-1 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold border whitespace-nowrap ${badge.color}`}
        >
          {badge.text}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
        {[
          ...
          <motion.div
            ...
            className="p-2 sm:p-4 rounded-lg bg-black/40 border border-emerald-500/20 text-center"
          >
            <div className="text-2xl sm:text-3xl md:text-4xl font-black text-emerald-400 font-mono mb-1 sm:mb-2">
              {String(item.value).padStart(2, "0")}
            </div>
            <div className="text-xs sm:text-sm text-emerald-200/70 uppercase tracking-wider">
              {item.label}
            </div>
          </motion.div>
        ))}
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs sm:text-sm text-emerald-200/60 text-right">
