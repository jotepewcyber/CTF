...
export default function LandingHeader() {
  ...
  const handleNavClick = (href: string) => {
    if (href.startsWith("#")) {
      // If it's a hash link (e.g., #aboutus)
              ...
              {navItems.map((item) =>
                item.href.startsWith("#") ? (
                  <button
                    ...
                    className="block w-full text-left px-4 py-2.5 text-emerald-200 hover:text-emerald-100 hover:bg-emerald-500/10 text-sm font-medium transition-colors"
                  >
                    {item.label}
                  </button>
                ) : (
